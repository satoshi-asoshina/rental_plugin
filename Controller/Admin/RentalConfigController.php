...
class RentalConfigController extends AbstractController
{
    ...
    private function getCurrentConfigData()
    {
        return [
            ...
            'default_delivery_fee' => $this->configRepository->get('default_delivery_fee', '500'),
            'free_delivery_amount' => $this->configRepository->get('free_delivery_amount', '5000'),
    ...
    private function resetToDefaultConfig()
    {
        $defaultConfigs = [
            ...
            'payment_methods' => '{"credit":"クレジットカード","bank":"銀行振込","convenience":"コンビニ決済","cash":"現金"}',
            'refund_policy' => '7',
        ...
        log_info('レンタル設定リセット完了', ['admin_user' => $this->getUser()->getId()]);
    }
     ...
     */
    private function validateImportData(array $configs)
    {
        if (!is_array($configs)) {
            throw new \InvalidArgumentException('設定データの形式が正しくありません。');
        }

        $requiredFields = ['config_key', 'config_value'];
        
        foreach ($configs as $config) {
            foreach ($requiredFields as $field) {
                if (!isset($config[$field])) {
                    throw new \InvalidArgumentException("必須フィールド '{$field}' が見つかりません。");
                }
            }
        }
    }
    ...
    private function validateAllConfigs()
    {
        ...
        try {
            ...
            // 追加の整合性チェック
            if ($configData['auto_approval'] && !$configData['deposit_required']) {
                $results['warnings'][] = '自動承認が有効ですが、保証金が無効になっています。';
            }

            if (!$configData['holiday_rental'] && count($configData['business_days']) < 5) {
                $results['warnings'][] = '休日レンタル無効で営業日が少なく設定されています。';
            }

            if (empty($configData['notification_email'])) {
                $results['info'][] = '通知メールアドレスが設定されていません。';
            }

            $results['info'][] = '設定の検証が完了しました。';
    ...
    private function getConfigStatistics()
    {
        return [
            ...
            'last_updated' => $this->configRepository->createQueryBuilder('c')
                ->select('MAX(c.update_date)')
                ->getQuery()
                ->getSingleScalarResult(),
            'auto_approval_enabled' => $this->configRepository->isAutoApprovalEnabled(),
            'deposit_required' => $this->configRepository->isDepositRequired(),
            'holiday_rental_enabled' => $this->configRepository->isHolidayRentalEnabled(),
        ];
    }
}